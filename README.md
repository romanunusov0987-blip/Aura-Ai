...
- `venv.ps1` — PowerShell-скрипт для создания виртуального окружения и установки зависимостей в Windows.
...
- **`venv.ps1`** — автоматизирует создание и настройку виртуального окружения в PowerShell.
...
### PowerShell: автоматическая настройка окружения

Если вы работаете в Windows, запустите в PowerShell команду `./venv.ps1`. Скрипт создаст (или обновит) виртуальное окружение в папке `.venv`, поставит/обновит `pip` и установит зависимости из `requirements.txt`. Параметры можно менять:

- `./venv.ps1 -VenvPath .venv-win` — создать окружение в другой папке.
- `./venv.ps1 -PythonExecutable py` — использовать конкретную версию интерпретатора (`py -3.11`, `C:\Python311\python.exe` и т. п.).

После выполнения скрипта активируйте окружение в текущей сессии командой `. .venv\Scripts\Activate.ps1` (или другой путь, если вы меняли `-VenvPath`).
