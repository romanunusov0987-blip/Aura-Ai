...
- `example.env` — актуальный шаблон файла настроек с пустыми значениями токенов и ключей.
- `.env` — личные секреты и настройки окружения (не хранится в репозитории, но создаётся на основе `example.env`).
...
- **`example.env`** — шаблон переменных окружения, который можно скопировать под своё окружение.
...
### Как подготовить `.env`
1. Скопируйте шаблон: `cp example.env .env`.
...
> ⚠️ Файл `.env` не хранится в репозитории, поэтому после клонирования проекта он всегда пустой. Ориентируйтесь на `example.env`, где уже есть строка `ADMIN_TELEGRAM_BOT_TOKEN=` для заполнения токена административного бота.

### Подготовка виртуального окружения

Создайте изолированную среду для зависимостей:

```powershell
python -m venv .venv
```

Активируйте её в PowerShell:

```powershell
. .venv\Scripts\Activate.ps1
```

Если PowerShell запрещает выполнение скриптов, временно разрешите локальные: `Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope Process`. После активации обновите `pip` и установите зависимости `pip install -r requirements.txt`.
