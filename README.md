...
- `requirements.txt` — список библиотек, которые нужно установить перед запуском (включает `fastapi`, `uvicorn` и другие ключевые пакеты).
- `tariff_ru.md` — текстовое описание тарифов и услуг, доступных пользователю бота.
- `powershell_launch_guide.txt` — подробный пошаговый гайд по запуску проекта через Windows PowerShell, включая сценарий «одной команды» и примеры alias.
...
- **`powershell_launch_guide.txt`** — готовый набор PowerShell-команд для быстрого запуска всех сервисов, плюс раздел про однокомандный старт и создание alias.
...
2. Установите зависимости: `python -m pip install --user -r requirements.txt`.
3. Запустите API: `python -m uvicorn Aura_Psycholog_bot:referral_api --reload`.
4. Документация будет доступна по адресу `http://127.0.0.1:8000/docs`.

> ❗ Если после запуска возникает `ModuleNotFoundError: No module named 'fastapi'`, значит зависимости не установлены в текущей среде Python. Проверьте, что вы активировали нужное окружение и выполнили команду `python -m pip install --user -r requirements.txt` (или `pip install -r requirements.txt` внутри виртуального окружения).
...
> ℹ️ Ошибка `ModuleNotFoundError: No module named 'fastapi'` при запуске основной команды всегда означает, что набор зависимостей из `requirements.txt` не установлен в используемое окружение. Повторите установку или убедитесь, что `pip` и `python` указывают на одну и ту же версию интерпретатора (на Windows это удобно проверить командами `where python` и `where pip`).
...
3. Реферальный FastAPI-сервис (`python -m uvicorn Aura_Psycholog_bot:referral_api`).

Перед запуском убедитесь, что заполнен `.env` и установлены зависимости (`python -m pip install --user -r requirements.txt`). По умолчанию API стартует на `0.0.0.0:8000`, параметры можно сменить через переменные `REFERRAL_API_HOST` и `REFERRAL_API_PORT`.
...
### 1. Установка зависимостей в системный Python
1. Убедитесь, что глобальный Python 3.11+ доступен в PATH (`python --version`).
2. Обновите менеджер пакетов: `python -m pip install --user --upgrade pip`.
3. Установите зависимости проекта для текущего пользователя: `python -m pip install --user -r requirements.txt`.
4. При необходимости добавьте директорию `~/.local/bin` (или `%APPDATA%\Python\Python311\Scripts`) в PATH, чтобы иметь доступ к установленным консольным утилитам.

> 💡 Для диагностики можно проверить, что `fastapi` действительно установлен: `python -m pip show fastapi`. Если пакет не найден, значит его нужно доустановить в текущий интерпретатор.

> ℹ️ Если вам всё же нужна изоляция зависимостей, можно создать виртуальное окружение вручную (`python -m venv .venv`) и активировать его перед установкой пакетов. Это полностью опционально и не требуется для запуска проекта.
...
2. Запускайте панель администрирования тем же системным Python: `python -m admin_bot`.
...
2. Запускайте API с авто-перезагрузкой: `python -m uvicorn Aura_Psycholog_bot:referral_api --reload`.
...
Базовый административный бот расположен в каталоге `admin_bot/` и использует те же зависимости, что и основной проект (установленные в системный Python).
...
2. Установите зависимости (если ещё не установлены): `python -m pip install --user -r requirements.txt`.
...
### Запуск в Windows (системный Python)

Проект рассчитан на работу с глобальным интерпретатором Python, поэтому отдельное виртуальное окружение не требуется:
...
4. Для запуска дополнительных сценариев ориентируйтесь на подсказки из `powershell_launch_guide.txt`: там описаны однокомандный старт через `python run_all.py`, настройка alias и варианты отдельных запусков скриптов (`python <имя_файла>.py`).
